...
Windows/macOS Coding Agent Host 需要同时约束工作区授权、代码索引、进程树、补丁副作用和崩溃恢复。能启动 shell 只是起点，生产验收看的是整条安全链能否审计和复现。文中的厂商与 OS 行为已于 **2026-08-30** 按官方资料核对；标注为“建议”的内容属于架构推导，不代表操作系统承诺。
