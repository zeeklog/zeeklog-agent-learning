...
Agent Runtime 会产生多类流：模型文字增量、推理/进度提示、工具调用参数、工具日志、审批请求、usage 与最终状态。它们的可靠性要求不同：
...
恢复应从最后一个已提交事实开始，不要重新发送最后一条用户消息：
...
测试应优先覆盖故障路径：在每个事件提交前后 kill worker；让 UI 每秒只消费一条而模型每秒生产百条；在 tool 成功、receipt 上报前断网；让 SSE 重复、乱序和缺失。
