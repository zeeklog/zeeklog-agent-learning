...
Multi-Agent 编排只有在委派可恢复、可审计且权限不扩张时才成立。本章给出 Supervisor–Worker 合同、预算托管、Context 与 Worktree 隔离、Join/Cancel 语义和冲突安全的交付路径。

## 1. 先判断是否真的需要多 Agent

多 Agent 的收益来自**隔离、专门化和可并行的证据生产**，不是角色扮演。只有任务可拆成低耦合子问题，且节省的关键路径时间大于编排、重复上下文和合并成本时才值得使用。
