...
- **Procedural（程序）**：做事方法，如“发布前运行哪些检查”；最好沉淀为版本化 Skill/Workflow，而非自由文本。
...
建立反馈闭环时要防自我强化：模型依据旧 Memory 生成回答，再把该回答作为新证据写回，会让错误置信度越来越高。派生内容必须追溯到独立原始证据，同源重复不增加置信度；没有新证据的模型复述不允许创建新的 Semantic Memory。
