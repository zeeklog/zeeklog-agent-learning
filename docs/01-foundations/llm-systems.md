# LLM 系统原理：Agent Runtime 必备的模型基础

## 1. 不需要训练模型，也必须理解推理约束

设计 Agent Runtime 不需要重新实现 Transformer，但模型的推理方式会直接改变成本、延迟、状态和失败边界。模型每次调用都在有限上下文上按 token 自回归生成结果。它不会自动记住上一次调用，也不是确定性函数；即使输入相同，模型版本、采样配置和服务端实现变化仍可能改变输出。
