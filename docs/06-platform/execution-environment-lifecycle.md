# 执行环境生命周期：从环境指纹到可证明清理

> Agent 的执行环境不是一个 `cwd`，而是有身份、策略、租约和证据链的基础设施资源。完整生命周期覆盖环境准备、云端两阶段执行、worktree、checkpoint、取消与数据删除。Codex 的产品行为按官方资料核对；其余内容作为企业级参考设计，仍需结合实际运行约束验证。
